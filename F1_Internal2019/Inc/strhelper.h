...
** 
** More resource efficient string conversion and printing functions
** 
 ...
 * 
 * If the number has less digits than specified then it is left--padded with zeros
 *
 * @param      data    The string to write to
...
static inline char* ftos(char *data, float a, uint32_t dp) {
	...
	*data++ = '.';
	...
	if (__dp != 0) data = utos0(data, (uint32_t)(d+0.5f), __dp);
	return data;
}
